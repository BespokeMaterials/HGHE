...
class Trainer:
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            self.evaluate(epoch, num_epochs, running_loss=0.7)
            ...
            for inputs in self.train_loader:
                ...
                pred = self.model(x=torch.randn_like(inputs.x.to(torch.float32)).to(self.device),
                                  u= torch.randn_like(inputs.u.to(torch.float32)).to(self.device),#inputs.u.to(torch.float32).to(self.device),
                                  edge_index=inputs.edge_index.to(torch.int64).to(self.device),
                                  edge_attr=torch.randn_like(inputs.edge_attr.to(torch.float32)).to(self.device), #inputs.edge_attr.to(torch.float32).to(self.device),
...
def ham_difference(pred, target, edge_index):
    ...
    print("Onsite pred:")
    v=3
    for i, xi in enumerate(onsite_p):
        print(f" ONSITE f {i}-{v}", torch.allclose(xi, onsite_p[v], atol=0.1, ))
    ...
    hop_d_scale = torch.norm(hop_dif * scale)
    penalized_difference = torch.norm((hop_dif) * torch.abs(hop_t*10))

    dif =onsite_dif # hop_d_scale + hop_d_scale  +  onsite_dif#+zero_count+penalized_difference**2
    print(f"onsite dif: {onsite_dif} | hop dif:{hop_dif}|hop_d_scale: {hop_d_scale}|penalized_difference:{penalized_difference}|total dif in module :{dif}|zc:{zero_count}")
    return dif
