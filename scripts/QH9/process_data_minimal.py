...
def main(dataset, radius, save_path):
    ...
    for i, g in enumerate(graphs):
        print(f"i-{i} e1:", g.data.edge_attr[1])
        print(f"i-{i} e2:", g.data.edge_attr[2])
        print(f"i-{i} e2-e1:", g.data.edge_attr[2]-g.data.edge_attr[1])
        print(f"i-{i} e2-e3:", g.data.edge_attr[2] - g.data.edge_attr[3])

        print(f"i-{i} n1:", g.data.x[1])
        print(f"i-{i} n2:", g.data.x[2])
        print(f"i-{i} n2-n1:", g.data.x[2] - g.x[1])
        print(f"i-{i} n2-n3:", g.data.x[2] - g.data.x[3])
...
if __name__ == "__main__":
    ...
    save_path = ("DATA/QH9/DFT_graphs_QH9_train_atomic_minimal"
                 ".pt")
